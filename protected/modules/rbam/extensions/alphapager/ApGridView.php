...
class ApGridView extends CGridView
{
	...
	public function renderAlphapager()
	{
		...
		if($this->ajaxUpdate!==false)
		{
			...
			{
				$cs->registerScript(__CLASS__.'#'.$id,"jQuery('#".$id." .".$this->alphaPagerCssClass." a').on('click',function(){ $.fn.yiiGridView.update('".$id."',{url: $(this).attr('href')});return false; });");
			}
			else if($class == 'ApListPager')
			{
				$cs->registerScript(__CLASS__.'#'.$id,"jQuery('#".$id." .".$this->alphaPagerCssClass." select').on('change',function(){ $.fn.yiiGridView.update('".$id."',{url: $(this).val()});return false; });");
